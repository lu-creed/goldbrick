...
def create_symbol(body: SymbolCreate, _admin=Depends(get_current_admin), db: Session = Depends(get_db)):
...
def patch_symbol(symbol_id: int, body: SymbolPatch, _admin=Depends(get_current_admin), db: Session = Depends(get_db)):
