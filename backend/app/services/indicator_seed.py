...
_SEED = [
    ...
    (
        "VWAP", "成交量加权均价",
        "以成交量为权重计算20日均价，反映主力参与成本。价格站上VWAP表示多方占优，跌破则空方主导。参数已锁定：N=20。",
        [("N", "滚动窗口（已锁定）", "20")],
        [
            ("VWAP", "20日成交量加权均价（元），常用于判断资金成本与多空力量对比", True),
        ],
    ),
    (
        "MFI", "资金流量指数",
        "结合价格与成交量的超买超卖指标，被称为「成交量版RSI」。取值0~100，>80超买、<20超卖。参数已锁定：N=14。",
        [("N", "计算周期（已锁定）", "14")],
        [
            ("MFI14", "14日资金流量指数，>80超买/<20超卖", False),
        ],
    ),
    (
        "StochRSI", "随机相对强弱指数",
        "将RSI值再做一次随机化处理，对超买超卖更敏感。K线上穿D线为买入信号，下穿为卖出信号。参数已锁定：RSI=14，K=3，D=3。",
        [
            ("RSI_N", "RSI计算周期（已锁定）", "14"),
            ("K",     "K线平滑周期（已锁定）", "3"),
            ("D",     "D线平滑周期（已锁定）", "3"),
        ],
        [
            ("StochRSI_K", "StochRSI快线（K），>80超买/<20超卖", False),
            ("StochRSI_D", "StochRSI慢线（D），K上穿D为金叉买入信号", False),
        ],
    ),
]
