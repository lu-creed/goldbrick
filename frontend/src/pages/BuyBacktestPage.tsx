import { Alert, Button, Card, DatePicker, Form, InputNumber, Select, Space, Spin, Table, Typography, message } from "antd";
import type { ColumnsType } from "antd/es/table";
import * as echarts from "echarts";
import { type Dayjs } from "dayjs";
import { useEffect, useMemo, useRef, useState } from "react";
import { fetchSymbols, getApiErrorMessage, runBuyOnceBacktest, type BacktestDailyPoint } from "../api/client";
...
export default function BuyBacktestPage() {
  ...
  const onRun = async () => {
    ...
    setError(null);
    try {
      const resp = await runBuyOnceBacktest({
        ts_code: v.ts_code,
      ...
      series: [
        {
          ...
          markPoint: buyDateMark
            ? {
                data: [{ name: "买入点", xAxis: buyDateMark, yAxis: rows.find((r) => r.trade_date === buyDateMark)?.total_asset }],
              }
            : undefined,
        },
    ...
    <Space direction="vertical" size="large" style={{ width: "100%" }}>
      ...
      <Card loading={loading}>
        <Form
          form={form}
          layout="inline"
          initialValues={{ buy_qty: 100, initial_cash: 100000 }}
      ...
      <Card title={`资产曲线（最大回撤 ${(maxDrawdown * 100).toFixed(2)}%）`}>
      ...
      <Card title="每日资产明细">
        <Table
          ...
          scroll={{ x: 900 }}
          onRow={(record) => ({
            style: record.trade_date === buyDateMark ? { background: "#fff7e6" } : undefined,
          })}
        />
      </Card>
    </Space>
